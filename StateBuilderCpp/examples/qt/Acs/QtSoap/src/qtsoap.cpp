...
QDomElement QtSoapStruct::toDomElement(QDomDocument doc) const
{
    ...
    QDomElement a = n.uri() == ""
		    ? doc.createElement(n.name())
		    : doc.createElement(prefix + ":" + n.name());
...
/*! 
    Add a namespace,
*/
void QtSoapMessage::namespaceAdd(const QString &prefix, const QString &uri)
{
  QtSoapNamespaces::instance().registerNamespace(prefix, uri);
  namespaceUriList.append(uri);
}
...
QString QtSoapMessage::toXmlString(int indent) const
{
    ...
    env.setAttribute("xmlns:" + QtSoapNamespaces::instance().prefixFor(SOAPv11_ENVELOPE), SOAPv11_ENVELOPE);
    env.setAttribute("xmlns:" + QtSoapNamespaces::instance().prefixFor(SOAPv11_ENCODING), SOAPv11_ENCODING);
    env.setAttribute("xmlns:" + QtSoapNamespaces::instance().prefixFor(XML_SCHEMA), XML_SCHEMA);
    env.setAttribute("xmlns:" + QtSoapNamespaces::instance().prefixFor(XML_SCHEMA_INSTANCE), XML_SCHEMA_INSTANCE);

    foreach(QString uri, namespaceUriList){
      QString prefix = QtSoapNamespaces::instance().prefixFor(uri);
      env.setAttribute("xmlns:" + prefix, uri);
    }
...
void QtSoapMessage::setHeader(const QtSoapStruct& header)
{
    const QtSoapQName headerName("Header", SOAPv11_ENVELOPE);

    QtSoapType &headerTmp = envelope[headerName];
    if (!headerTmp.isValid()){
      envelope.insert(new QtSoapStruct(header));
    } else {
      qWarning("Soap header already set");
      envelope[headerName] = header;
    }
