...
HttpParser::parseHeader()
{
  ...
  static const struct FSM {
    ...
    { p_key_colon_sp,         CR, p_value_cr,            0                                  },
  ...
  for ( int i = _parsedTo; i < _data.length(); ++i) {
    char c = _data[i];
...
HttpParser::status_t
HttpParser::addBytes(QByteArray &bytes)
{
...
{
  if ( _contentLength > 0 ) {
    return &_data.data()[_contentStart];
  } else  {
    return NULL;
  }
}
...
HttpParser::getValue( const char* key )
{
  for ( IntArray::iterator iter = _keys.begin();
    ...
    if ( qstrnicmp( &_data.data()[index], key, qstrlen(key)) == 0 ) {
      return &_data.data()[index + strlen(key) + 2];
    }
  }
